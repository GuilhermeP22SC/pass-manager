...
// Preenchimento de Login (usa setNativeValue)
...
function setNativeValue(element, value) {
  ...
  // Tenta usar o setter mais nativo/seguro (bypassa React/Vue overrides)
  try {
    // 1. Pega o setter nativo do protótipo do input para bypassar overrides do React
    ...
    // Fallback simples se a manipulação de protótipo falhar
    element.value = value;
  }

  // Hack para React (o tracker precisa ser setado antes do 'input' event)
  const tracker = element._valueTracker;
  if (tracker) {
    tracker.setValue(lastValue);
  }

  // Dispara eventos em sequência para garantir o processamento por frameworks
  element.dispatchEvent(new Event('input', { bubbles: true }));
  element.dispatchEvent(new Event('change', { bubbles: true }));
  element.dispatchEvent(new Event('blur', { bubbles: true })); // Importante para validação de forms
}
